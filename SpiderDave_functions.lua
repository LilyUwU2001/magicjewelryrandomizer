...
function to display memory on screen; good for debugging n stuff
    example usage:
        showmem(0x1234)  --use this ONLY if you're using address alone
        showmem{address=0x1234, x=0,y=0} --note the special {} syntax!
        showmem{0x1234, 0, 0} --this works too
    parameters:
        address, x, y
--------------------------------------------------]]--
function showMem(args)
    ...
    if (type(args)=="number") then
        ...
        address=args.address or args[1] or 0
    end
    
    local _address,x,y
    
    x=args.x or args[2] or 8
    y=args.y or args[3] or 8
